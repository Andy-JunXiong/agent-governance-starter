"""Installed deterministic, privacy-bounded task-start scope baselines.

This module captures the already-dirty Git state at an explicit task boundary and
...
def task_start_baseline_path(task_id: str) -> str:
    """Return the one canonical local baseline path for a task identity."""

    if not isinstance(task_id, str) or not re.fullmatch(
        r"^[a-z0-9]+(?:-[a-z0-9]+)*$", task_id
    ):
        raise BaselineError("task identity cannot form a canonical baseline path")
    return f"{LOCAL_BASELINE_PREFIX}/{task_id}.json"
    ...
    *,
    ...
    return BaselineComparison(
        ...
        known_limits=(
            ...
            "the local baseline grants no task, exception, Git, or release authority",
        ),
        authority_boundary={
